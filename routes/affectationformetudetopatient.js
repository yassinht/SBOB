...
router.post('/addaffectation', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let affectation = new Affectationformetudetopatient(obj);
        affectation.date = new Date();
        affectation.dateRemplissage = null;
        affectation.etat = false;
        let aff = await Affectationformetudetopatient.findOne({ user: obj.user, form: obj.form, doctor: obj.doctor })
...
router.get('/getaffectation', verifyToken, async (req, res) => {
    ...
    try {
        let affectations = await Affectationformetudetopatient.find()
...
router.get('/getaffect/:user/:form', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let affectation = await Affectationformetudetopatient.findOne({ user: req.params.user, form: req.params.form })
        if (!affectation) {
...
router.get('/getallform/:user/:doctor', verifyToken, async (req, res) => {
    try {
        ...
        let affect = await Affectationformetudetopatient
            .aggregate(
                [
                    {
                        $match: {
        ...
        for (let i = 0; i < affect.length; i++) {
            let obj = {
                ...
                form: affect[i].formetudes[0]
            }
            if(affect[i].formetudes[0]){

                if (!affect[i].formetudes[0].archived) {
        ...
        let affect1 = await Affectationformetudetopatient
            .aggregate(
                [
                    {
                        $match: {
        ...
        for (let i = 0; i < affect1.length; i++) {
            let obj = {
                ...
                form: affect1[i].formetudes[0]
            }
            if(affect1[i].formetudes[0]){
                if (!affect1[i].formetudes[0].archived ) {
...
router.get('/getmyincompletedform/:user', verifyToken, async (req, res) => {
    try {
        ...
        let affectations = await Affectationformetudetopatient
            .aggregate(
                [
                    {
                        $match: {
...
router.delete('/deleteaffectation/:id', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let affectation = await Affectationformetudetopatient.findByIdAndDelete({ _id: id })
...
router.put('/updateaffectation/:id', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let affectation = await Affectationformetudetopatient.findByIdAndUpdate({ _id: id }, { $set: { etat: true } })
