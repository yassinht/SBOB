...
const { FormEtude } = require('../models/formEtude');
...
router.post('/', async (req, res) => {
  try {
    let obj = req.body;
    let dossier = new DossierFormEtude(obj);
    try {
      let dos = await DossierFormEtude.find({ name: req.body.name }); 
      ...
      // si inside folder 
      // idDossier = 6231dcc2b33fb905e0a040fb,
      // si main folder 
      // iddossier= sddddddddddddddgsth25
      // are static we send it from front office every time
...
router.get('/getDossier/:id', verifyAdminToken, async (req, res) => {
  /*   console.log(1)  */
    try {
      ...
      let dossier = await DossierFormEtude.findOne({ idDossier: id, archived: false })
...
router.get('/' , async (req, res) => {
  try {
    let dossiers = await DossierFormEtude.find({ archived: false }).sort({ 'name': 1 })
...
router.put('/:id', verifyAdminToken, async (req, res) => {
  try {
    ...
    let updateddossier = await DossierFormEtude.findByIdAndUpdate({ _id: id }, data, { new: true })
...
router.delete('/:id', verifyAdminToken, async (req, res) => {
  try {
    ...
    let dossier = await DossierFormEtude.findByIdAndDelete({ _id: id })
...
router.get('/archivedSousDossier/:id', verifyAdminToken, async (req, res) => {
    try {
      let id = req.params.id;
      let updatedForms = await DossierFormEtude.findByIdAndUpdate({ _id: id }, { $set: { archived: true ,dossierAff:[{nameDossier:"Aucune dossier",id:""}],} })
...
router.get('/restorer/:id', verifyAdminToken, async (req, res) => {
  try {
    ...
    let updatedForms = await DossierFormEtude.findByIdAndUpdate({ _id: id }, { $set: { archived: false } })
...
router.get('/archive/getdossierfromarchive', verifyAdminToken, async (req, res) => {
  try {
    let dossiers = await DossierFormEtude.find({ archived: true })
    res.status(200).send(dossiers);
  } catch (error) {
    res.status(400).send({ message: "Erreur", error });
  }
